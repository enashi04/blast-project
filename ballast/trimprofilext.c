...
if (getargfloat ("-sens",&sensitivity) == NULL) sensitivity = D2MAX;
...
/*
printf ("%4d %c : %5.2f %5.2f (%5.2f) %5.2f ",i,*(cons +i),*(profil+i),*(smoothed+i),*(dprof+i),*(d2prof+i));
*/

	if ( *(d2prof + i) < sensitivity) 
		{
		if (first == 0) {/*printf ("first ");*/ first = i;}
/*
		printf ("last ");
*/
		last  = i;
...
*/
	if ( (i>0) && (first*last != 0) && (*(smoothed + i ) < 0 ) && (*(smoothed + i +1) >= 0 ))
		{
		m = 0;
		for (j = first; j <= last; j++) { if (m < *(profil+j)) m = *(profil+j);}
