...
while ((insert != NULL) && (insert->prob <= ELIMIT))
	...
	if (insert != NULL)
		...
		if (insert->sortedprev != NULL) { printf ("           était entre %s et ",insert->sortedprev->name);}
		else { printf ("           était au début, juste avant ");}
...
while (seqres != NULL)
	...
	if (seqres->sortedprev != NULL) { printf ("           était entre %s et ",seqres->sortedprev->name);}
	else { printf ("           était au début, juste avant ");}
	...
	else { printf ("           ====> au début, juste avant ");}
	if (seqres->sortednext != NULL) { printf ("%s\n",seqres->sortednext->name); }
	else { printf ("la fin\n"); }
