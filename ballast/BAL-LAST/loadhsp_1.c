...
void initSimprf(SeqHSP *seqres, SimPrf *simprf, double maxp, char type, double *profil, double *ptr, int length);
...
double *loadHSP(SeqHSP *seqres, FILE *file, char *line, int length, char *conserved, double *maxprofile, char type)
{
	...
	printf("on récupération de maxp\n");
	...
	printf("Allocation de la mémoire de outtext\n");
	...
	ptrstr = strtok(NULL, " ");
	printf("Initialisation de seqAccess\n");
	...
	while (endofdbseq == 0)
	{
		...
		printf("On est dans la fin de blast\n");
...
void seqAccess(char *ptrstr, SeqHSP *seqres)
{
	if (ptrstr != NULL)
	{
		...
		printf("On est dans seqAccess : %s\n", seqres->access);
	}
	else
	{
