...
#ifdef WWW
if (getargchar ("-mask",&maskfilename) == NULL) maskfile = fopen (maskfilename,"w");
...
for (seqres = first; seqres != NULL; seqres=seqres->next)
        {
        if ((seqres->type == 'n') && (both == FALSE)) break;
...
for (seqres = first; seqres != NULL; seqres=seqres->next)
	{
	if (seqres->prob >= 1) continue;
	if ((seqres->type == 'n') && (both == TRUE) && (seqres->p != 1)) continue;
	if ((seqres->type == 'p') && (seqres->p != 1)) continue;
	fprintf (listfile,"%s ",seqres->desc);
	printnicedouble(listfile,seqres->prob);
	fprintf (listfile,"\n");
	}
...
for (i=0;i<length;i++)
	...
	*/
	if (max < *(maxprofile + i)) max = *(maxprofile + i);
	}
...
for (i=0;i<length;i++)
	...
	if (*(profil2 +i) > 0.0)
		{
		for (j=0;j<k;j++) fprintf (plotfile,"#");
		}
	else
		{
		for (j=0;j<k;j++) fprintf (plotfile,"+");
	...
	k = (int) (*ptr / scale)-k;
	for (j=1;j<k-1;j++) fprintf (plotfile," ");
	fprintf (plotfile,"|");
#else
...
#ifdef WWW
fclose (listfile);
if (getargchar ("-mask",&maskfilename) == NULL) fclose (maskfile);
#endif
