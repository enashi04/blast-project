...
void tag_value(void *data, const char *text, int len)
{
	...
	if (state.access == 2) //pour la longueur de l'alignement
	{
		fprintf(output, "%s\n", content);
		strcpy(content, state.query);
		state.access = 0;
	}
	if (state.access == 3) //pour la définition de la query
	{
		fprintf(output, "Query-def :%s\n", content);
		strcpy(content, state.query);
		state.access = 0;
	}
	if (state.access == 4) //pour la longueur de la query
	{
