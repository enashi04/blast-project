...
double MHalign(Motif *motif, Sbjmot *sbjmot, double *profil, char type, Sbjmot **first)
{
...
#endif
					sensepenalty = 0.0;
					if (sbjmotptr->hsp->sens != previous->hsp->sens)
						sensepenalty = SPENALTY;
#ifdef DEBUG
	...
	while (sbjmotptr != NULL)
	{
		if (sbjmotptr->alnscore > best->alnscore)
			best = sbjmotptr;
		sbjmotptr = sbjmotptr->sortednext;
	}
	...
	while (sbjmotptr != NULL)
	{
		...
		*first = sbjmotptr;
		if (sbjmotptr->alnprev != NULL)
			sbjmotptr->alnprev->alnnext = sbjmotptr;
