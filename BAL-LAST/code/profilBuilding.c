...
double *profilBuilding(SeqHSP *seqres, FILE *file, char *line, int length, char *conserved, double *maxprofile, char type)
{
    ...
    seqres->prob = p;
    simprf->p = p;
    /*Condition fixant la e-value à 1*/
    fixeValue(p, maxp);
    /**Formule de calculs***/
    facteur = (1.0) * p;
    fctr = 1;
    ...
    while (endofdbseq == 0)
    {
        ...
        if (strncmp(line, " Identities", 11) == 0)
        {
            ...
            /*Récupération du taux et du nbre d'identité*/
        ...
        if (strncmp(line, "Query", 5) == 0)
        {
            ...
            /*Recherche du numérodu début et fin de la query*/
            
            sscanf((char *)(strpbrk(line, "0123456789")), "%d", &debut);
            sscanf((char *)(strrchr(line, ' ')), "%d", &end);
            
            /*Initialisation de la dline qui correspond à la longueur qui le début de la séquence (autour de 12)*/
            
            dline = strlen(line) - strlen(strpbrk(line +1, "ABCDEFGHIJKLMNOPQRSTUVWXYZ-"));
            
            /*Copier la nouvelle valeur de la ligne en ne laissant que la séquence*/
            ...
            *ptrstr = '\0';
            ...
            /*Condition permettant de vérifier si nous sommes à la premièrepartie de la séquence de la midline ou non*/
            line[strlen(line)] = '\0';
        ...
        if (strncmp(line, "Sbjct", 5) == 0)
        {
            ...
            /*Condition permettant de vérifier si la séquence de la subject est nulle ou non*/
            if (startline == NULL)
            {
                line[0] = '\0';
                endofdbseq = 1;
        ...
        /**** This is supposed to be the end of the BLAST results, we don't need to read the file any further and may leave*/
